...
public class AgenciaDAO {
    ...
    public void inserir(Agencia agencia) {
        ...
        sql = "INSERT INTO AGENCIA (NRO_AGENCIA, COD_BANCO, NOME, DIG_AGENCIA) VALUES (?, ?, ?, ?)";
        try {
    ...
    public List<Agencia> listar(Banco banco) {
        ...
        sql = "SELECT * FROM AGENCIA WHERE COD_BANCO = ?";
        try {
            ps = con.prepareStatement(sql);
    ...
    public boolean pesquisar(String codBanco, String numAgencia) {
        Conexao conexao = new Conexao();
        con = conexao.conectar();
        sql = "SELECT * FROM AGENCIA WHERE NRO_AGENCIA = ? AND COD_BANCO = ?";
    ...
    public void alterar(Agencia agencia) {
        con = new Conexao().conectar();
        sql = "UPDATE AGENCIA SET NOME = ? WHERE NRO_AGENCIA = ? AND COD_BANCO = ?";
        try {
            ps = con.prepareStatement(sql);
